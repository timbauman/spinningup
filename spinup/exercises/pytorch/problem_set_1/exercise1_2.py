...
def mlp(sizes, activation, output_activation=nn.Identity):
    ...
    for i in range(1, len(sizes)):
        ...
        layers.append(activation() if i < len(sizes) - 1 else output_activation())
...
class DiagonalGaussianDistribution:
    ...
    def sample(self):
        ...
        """
        return torch.distributions.Normal(loc=self.mu, scale=torch.exp(self.log_std)).sample()
...
class MLPGaussianActor(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation):
        ...
        """
        self.log_std = torch.tensor([-0.5]).expand(act_dim).clone()
